...
export const ColorInput = memo(function ColorInput() {
  ...
  return (
    <div className="flex flex-col justify-center items-center mt-20">
      <div className="bg-base-200 shadow-md rounded-lg p-4 max-w-lg flex flex-wrap gap-4">
        <div className="flex-1 flex-shrink-0 flex flex-col min-w-[250px]">
          <label htmlFor="background" className='mb-2'>{t('label-background')}</label>
          <Controller
            name="background"
            render={({ field }) => {
        ...
        <div className="flex-1 flex-shrink-0 flex flex-col min-w-[250px]">
          <label htmlFor="foreground" className='mb-2'>{t('label-foreground')}</label>
          <Controller
            name="foreground"
            render={({ field }) => {
              return <HsColorPicker {...field} />
