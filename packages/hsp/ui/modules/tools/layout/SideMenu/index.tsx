...
const ToolsGroup = memo(function ToolsGroup({ item }: { item: MenuItemGroup }) {
  ...
  return (
    <li>
      <h6
        className={
          'px-4 py-2 bg-base-100 flex justify-center items-center menu-title rounded'
        }
      >
...
const ToolsGroupLink = memo(function ToolsGroupLink({
  ...
  return (
    <Link
      className={classNames(
        'btn btn-neutral flex flex-col justify-center items-center px-2 py-3 flex-1 min-w-[40%] h-full relative',
        isSelected && 'btn-active'
      )}
      title={t(item.key)}
      href={item.href}
